<template>
  <div class="space-y-5">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-gradient-to-br from-brand-50 to-white rounded-2xl border border-brand-100 p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div><p class="metric-label">Total Leads</p><p class="metric-value">{{ totalLeads }}</p></div>
    ...
    <div class="bg-gradient-to-br from-brand-50 to-white rounded-2xl border border-brand-100 shadow-sm overflow-hidden">
      <div class="px-5 py-4 border-b border-brand-100 flex items-center justify-between">
        ...
        <router-link to="/bank/campaigns/create" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> New</router-link>
      </div>
      <div class="p-5">
        <table class="tbl-shadcn">
          <thead><tr><th>Title</th><th>Status</th><th>Leads</th></tr></thead>
          <tbody>
...
const totalLeads = computed(() => campaigns.value.reduce((s, c) => s + (c.completions || 0), 0))
