    ...
    <section class="pt-28 pb-10 relative overflow-hidden" style="background: linear-gradient(135deg, #FF7A30 0%, #ea6a24 100%);">
      <div class="absolute inset-0 opacity-10">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs><pattern id="hex-s" x="0" y="0" width="56" height="97" patternUnits="userSpaceOnUse">
      ...
      <div class="max-w-[1440px] mx-auto px-6 relative z-10">
        <span class="text-xs font-semibold text-yellow-300 uppercase tracking-widest">Sponsor Portal</span>
        <h1 class="text-3xl font-bold text-white mt-1">{{ pageTitle }}</h1>
        <p class="text-white/70 text-sm mt-1" v-if="$route.name === 'sponsor-dashboard'">Overview of your campaigns and wallet.</p>
        <p class="text-white/70 text-sm mt-1" v-else-if="$route.name === 'sponsor-campaigns'">Manage your advertising campaigns.</p>
        <p class="text-white/70 text-sm mt-1" v-else-if="$route.name === 'sponsor-campaigns-create'">Create a new campaign.</p>
        <p class="text-white/70 text-sm mt-1" v-else>Deposit and manage your funds.</p>
      </div>
    </section>
...
const menu = [
  { path: '/sponsor/dashboard', label: 'Dashboard', icon: 'fas fa-chart-pie' },
  { path: '/sponsor/campaigns', label: 'Campaigns', icon: 'fas fa-ad' },
  { path: '/sponsor/wallet', label: 'Wallet', icon: 'fas fa-wallet' },
]

const pageTitle = computed(() => {
  const map = { 'sponsor-dashboard': 'Dashboard', 'sponsor-campaigns': 'Campaigns', 'sponsor-campaigns-create': 'New Campaign', 'sponsor-wallet': 'Wallet' }
  return map[route.name] || 'Sponsor'
})
